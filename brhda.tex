...
\begin{definition}
  An \emph{ST-pomset} is a pomset $(P, {<}, {\evord}, \lambda)$ on $\Sigma^\ST$ such that for all $x\in P$,
  $S_{\lambda(x)}=\bigotimes_{y\ll x} T_{\lambda(y)}$ and
  $T_{\lambda(x)}=\bigotimes_{x\ll y} S_{\lambda(y)}$.
\end{definition}
\aain{This suggests that the sources of $\min(P)$ and the targets of $\max(P)$  are empty. They should be arbitrary. }
\aain{May be write $S_x$ instead of $S_{\lambda(x)}$ ?}
...
\end{itemize}
Every non-empty sp-pomset $P$ is \aasb{uniquely}{I believe this is not true because of the associativity of the two products} represented by a term $t(P)$ 
of series and parallel products of singleton pomsets.\ufsb{}{citation needed}
...
\aain{Interfaces are missing in the base case ?}
\begin{itemize}
\item $\ST(\emptyset)=\emptyset$; $\ST(a)=a$ for $a\in \Sigma$;
...
\aain{This does not generalize general branching automata but only fork acyclic ones : those do not containing nested forks (and accepting bounded-width sp pomsets) In particular, in the second item above $q_i's$ must be different from $q$, otherwise  $\ev(q)=\bigotimes_{i=1}^n \ev(q_i)$ does not hold.}
...
\aain{Lodaya and Weil required at least two branches} 	
}
