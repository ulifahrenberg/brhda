...
\end{lemma}
\begin{proof}
	This is shown in \cite{FJSZ_Kleene} + the observation that interval iposets are closed with respect $+$, $*$, $(-)^*$. (So we do not get and gp-posets that are not interval).
\end{proof}

\begin{proposition}
	Every regular gp-language is rational.
\end{proposition}
\begin{proof}
	Let $X$ be a BHDA. By  $L_0(X)_x^y$ is a regular interval language and by the Lemma above, a rational gp-language.
TBC
\end{proof}
